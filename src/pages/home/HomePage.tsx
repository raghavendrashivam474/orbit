...
export function HomePage(): React.JSX.Element {
  ...
  return (
    <div className="min-h-full flex flex-col items-center px-6 py-12 animate-fade-in">
      {/* Logo + greeting */}
      ...
      {/* Two-column area: Recent + Bookmarks */}
          ...
          <div className="orbit-panel p-2 space-y-0.5 min-h-[180px]">
            {recent.length === 0 ? (
              ...
              recent.map((entry) => (
                <button
                  key={entry.id}
                  onClick={() => handleQuickOpen(entry.url)}
                  className={[
          ...
          <div className="orbit-panel p-2 space-y-0.5 min-h-[180px]">
            {topBookmarks.length === 0 ? (
              ...
              topBookmarks.map((bm) => (
                <button
                  key={bm.id}
                  onClick={() => handleQuickOpen(bm.url)}
                  className={[
                    "w-full flex items-center gap-2 px-2 py-1.5",
                    "rounded-[var(--radius-sm)] text-left",
                    "hover:bg-[var(--elevated)]",
